...
- Help derives positional arguments, required option groups, allowed values and public short forms from the parser and public vocabulary. Canonical command paths are stored on the parser tree, independently of argparse's version-dependent program/usage text. Recovery guidance names the actual flag or GUI control. `DestinationExistsError` carries a storage conflict to the CLI, which explains `--overwrite` and an unused `--output` path; storage itself has no command syntax. This also handles a destination created while an exclusive save is being staged.
