...
1. **Define the school timetable.** Fresh checkouts have no installed semesters. Examples under `examples/semesters/` are explicit opt-in templates. `--write --semesters --new` creates independent shared files. A blank timetable is a draft; an imported or copied timetable is validated before its folder is installed. Block names/counts, intervals, weekday patterns and duration options are data, not CLI constants.
...
| --- | --- |
| `semester_editor.py` | Shared in-memory definition edits; whole-definition/profile validation, stale checks, backups and rollback on save errors |
| `semester_cli.py` / `semester_gui.py` | Argument/prompt and Tk form adapters over the shared editor |
...
Timetable edits stage a candidate outside the data root, load it through the same semester validator and validate all dependent profiles without rewriting them. Each changed definition file is atomically replaced with a sibling backup; a caught write error restores earlier replacements. This is not a crash-atomic multi-file transaction. Session IDs are preserved, including previously generated IDs when rows are reordered. Inspection snapshots remain exact historical previews; inspect again after edits before exporting updated events.
