...
## CI portability fixes — 2026-09-22

Canonical command paths now live on the parser tree instead of being derived
from argparse's program text. This fixes Python 3.11 help crashes and missing
shortcuts caused by custom usage placeholders appearing in child program names.
A regression test covers nested aliases, custom usage and renamed entry points.
CLI test roots now resolve temporary-directory aliases just as Workspace does,
fixing inspection-cache comparisons and GUI launch mocks on macOS `/var` aliases
and Windows short paths.

Applied these fixes to the latest `main` in an isolated worktree, preserving the
package rename and existing local editor work. **152 tests passed** on Python
3.14.2 with `SHBS_GUI_TESTS=1`, including native Tk and independent calendar-parser
checks. On Python 3.13, **152 tests ran successfully, 18 skipped** (16 opt-in GUI
and 2 unavailable independent-parser tests), with a deliberately noncanonical
temporary path containing `..` to exercise path normalization.

Reviewed the exceptions-help screenshot generated by `tools/verify_preview.py
--help-page exceptions`; commands and time filters are readable. The temporary
window closed, bytecode generation was disabled and no personal data or global
packages were changed. `git diff --check` passed. Python 3.11 and native macOS
remain for the GitHub Actions matrix to verify.
